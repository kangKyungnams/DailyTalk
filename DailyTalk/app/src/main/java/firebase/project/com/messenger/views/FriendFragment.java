...
public class FriendFragment extends Fragment {
    ...
    @BindView(R.id.frag_friends_add)
    Button bt_add;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        // 1. 리얼타임데이터베이스에서 나의 친구목록을 리스터를 통하여 데이터를 가져옴.
        addFriendListener();
        // 2. 가져온 데이터를 통해서 recyclerview의 아답터에 아이템을 추가 시켜준다. (UI)갱신
    ...
    public void addFriend(){
        ...
        // 2. 이메일이 입력되지 않았다면 이메일을 입력해주시라는 메세지를 띄워줌
        if ( inputEmail.isEmpty()) {
            Snackbar.make(mSearchArea, "이메일을 입력해주세요. ", Snackbar.LENGTH_LONG).show();
            return;
        }
        // 3. 자기 자신을 친구로 등록할 수 없기때문에 FirebaseUser의 email이 입력한 이메일과 같다면, 자기자신은 등록 불가 메세지를 띄워줌
