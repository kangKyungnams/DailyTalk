...
import firebase.project.com.messenger.adapters.ChatListAdapter;
import firebase.project.com.messenger.customviews.RecyclerViewItemClickListener;
import firebase.project.com.messenger.models.Chat;
import firebase.project.com.messenger.models.ExitMessage;
import firebase.project.com.messenger.models.Message;
import firebase.project.com.messenger.models.Notification;
...
public class ChatFragment extends Fragment {
    ...
    private void addChatListener() {
        mChatRef.addChildEventListener(new ChildEventListener() {
            ...
            public void onChildChanged(final DataSnapshot chatDataSnapshot, String s) {
                ...
                // lastMessage ->last 메세지의 시각과 변경된 메세지의 last메세지 시간이 다르다면 -> 노티피케이션을 출력.
                // 현재 액티비티가 ChatActivity 이고 chat_id 가 같다면 노티는 해주지 않는다.

                // ui 갱신 시켜주는 메서드로 방의 정보를 전달.
                // total unread의 변경, title의 변경, lastmessage 변경시에 호출됨
                // 방에 대한 정보를 얻어오고
                ...
                if (updatedChat.getLastMessage() != null) {
                    ...
                    if (!updatedChat.getLastMessage().getMessageUser().getUid().equals(mFirebaseUser.getUid())) {// 노티피케이션 알림
                        if (!updatedChat.getChatId().equals(JOINED_ROOM)) {
                            Intent chatIntent = new Intent(mContext, ChatActivity.class); //context로 이동
    ...
    public void leaveChat(final Chat chat) {
        ...
        Snackbar.make(getView(), "선택된 대화방을 나가시겠습니까?", Snackbar.LENGTH_LONG).setAction("예", new View.OnClickListener() {final DatabaseReference messageRef = mFirebaseDatase.getReference("chat_messages").child(chat.getChatId());
                ...
                mChatRef.child(chat.getChatId()).removeValue(new DatabaseReference.CompletionListener() {
                    ...
                    public void onComplete(DatabaseError databaseError, DatabaseReference chatRef) {
                        ...
                        mChatMemberRef
                         ...
                         // 대화방의 타이틀이 변경됨을 알려주어 채팅방의 리스너가 감지하게 하여 방 이름을 업데이트 하게 해야함
                         // 방의 제목은 방의 업데이트, 추가되었을때의 리스너가 처리하기때문에 방 제목만 변경 시켜서 변경이 되었음만 알리면 됨
